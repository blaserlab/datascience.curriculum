...
There are two ways to install packages into your project:  

* ```renv::install()``` goes to the repository or github site and downloads a new or updated copy into your cache, then links the latest version into your project.  Can be very slow and frustrating

* ```renv::hydrate()``` gets the latest version available from your cache and links that one.  Always faster than install but won't update and will give you an error if you don't have it on your system already. 
...
* tip:  When you run renv::init(), anything inside ```library()``` will get hydrated.  So if you have packages you use all the time, add them in there.
