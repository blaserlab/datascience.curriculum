...
This lecture is on building R data packages. This is a relatively
...
- raw data: should be unchanged after creation. Can be entirely
  ...
  Larger files should be archived on a shared network drive.
...
things like Mendeley, Zotero, and Dryad to share large data packages.
Each has a straightforward web interface and the option to keep data
private until the manuscript is published.
...
- Load the data package back into your project using
  [`blaseRtemplates::project_data()`](https://blaserlab.github.io/blaseRtemplates/reference/project_data.html).
